...
#include "Sounds/Gong.wav.h"
#include "Sounds/OceanWave.wav.h"
#include "Sounds/DanceEnergy.wav.h"
#include "Sounds/VideoGame.wav.h"

// The following has been taken from main.h from LiveSynthesisU. These are not the same as the wut sndcore2.
// These are necessary for sound to work correctly. It took me several hard days of trial and error to determine
...
struct AXVoice
{
	...
	//! The priority of this voice used for force-acquiring a voice (set a default by sound.c code).
	uint32_t priority;
	...
	//! A bitfield representing different things needing to be synced (not used by sound.c code).
	uint32_t syncBits;
...
// These are different from voice.h, these must be used for sound to operate correctly.
