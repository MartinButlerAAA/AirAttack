...
#include "Sounds/SoundData.h"		// For the voice.h functions and structures, and header files for sounds.
#include "Sounds.h"					// For the sound functions used by the game and the enumerated values.
...
void axFrameCallback()
{
	...
	// Play music while waiting to start game.
	if (soundSel == BKGNDWAIT)
	{
		...
		voiceOffsets.endOffset = (384000 - 1);					// Loop end at end of data (array size minus 1 see sound data header file).
		...
		voiceOffsets.data = (unsigned char*)DanceEnergyWave;	// Pointer to the actual sound data.
		...
		putsoundSel(NOCHANGE);									// Clear soundSel to avoid repeating action.
	}
	// Play music for game playing.
	if (soundSel == BKGNDPLAY)
	{
		AXSetVoiceState(p_vceBackground, 0);					// Set state to stopped while making changes.
		...
		voiceOffsets.loopingEnabled = 1;						// Loop enabled for background music to keep looping round.
		voiceOffsets.loopOffset = 0;							// Loop offset at very start.
		voiceOffsets.endOffset = (327662 - 1);					// Loop end at end of data (array size minus 1 see sound data header file).
		voiceOffsets.currentOffset = 0;							// Set to start of data (although doesn't seem to be used).
		// Seems to be important that this is an unsigned char pointer (rather than void), for sound to actually work.
		voiceOffsets.data = (unsigned char*)VideoGameWave;		// Pointer to the actual sound data.
		AXSetVoiceOffsets(p_vceBackground, &voiceOffsets);		// Pass the new voice data into the voice.

		AXSetVoiceState(p_vceBackground, 1);					// Set state to playing.
		putsoundSel(NOCHANGE);									// Clear soundSel to avoid repeating action.
	}
	// Play sound for bullet firing.
	if (soundSel == FIRE)
	{
		...
		voiceOffsets.endOffset = (47883 - 1);					// Loop end at end of data (array size minus 1)
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)GongWave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);

		AXSetVoiceState(p_vceEffect, 1);
		putsoundSel(NOCHANGE);
	}
	// Play sound for plane hit.
	if (soundSel == HIT)
	{
		AXSetVoiceState(p_vceEffect, 0);
		...
		voiceOffsets.loopOffset = 0;
		voiceOffsets.endOffset = (31808 - 1);
		voiceOffsets.currentOffset = 0;
		voiceOffsets.data = (unsigned char*)OceanWaveWave;
		AXSetVoiceOffsets(p_vceEffect, &voiceOffsets);
...
void setupSound()
{
	...
	// Set background to lower volume so effects stand out.
	vol = 0x30000000;
	...
	putsoundSel(BKGNDWAIT);	// Start the background music.
	return;
}
